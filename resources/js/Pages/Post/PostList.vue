...
const handleSelectAll = () => {
    ...
    if (form.all_selected === true) {
        for (let i in props.posts.data) {
            form.list_check.push(props.posts.data[i].id)
            console.log(props.posts.data[i].id)
        }
    } else {
        form.list_check = []
    }
...
const handleAction = (action) => {
    router.post(route('admin.post.action'), {
        action: action,
...
<template>
    <div>
        ...
        <AuthenticatedLayout>
            <div>
                <p class="mb-5 dark:text-white text-2xl">Tin tức</p>
            </div>
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="(post, index) in posts.data" :key="index"
                                ...
                                <td>
                                    <div class="px-6 py-3">
                                        <span class="text-xs font-semibold dark:text-white">{{ post.title }}</span>
                                    </div>
                                </td>

                                <td scope="row">
                                    <div class="ps-3 dark:text-white">
                                        <span class="text-normal font-semibold">{{ post.user.name }}</span>
                                ...
                                <td class="px-6 py-4 ">
                                    <div class="flex items-center">
                                        <Link
                                            :href="route('admin.post.edit', post.id + `?back_to=${encodeURIComponent(page)}`)"
                                            v-if="queryParam != 'trash'" type="button"
