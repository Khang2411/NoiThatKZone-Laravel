...
const handleRemove = (id) => {
    if (confirm("Bạn có muốn xóa?")) {
        router.post(route('admin.category.delete', id), {
        }, {
            preserveScroll: true,
            onSuccess: () => {
                toast.remove(toastId.value)
...
<template>
    ...
    <AuthenticatedLayout>
        <div>
            <p class="mb-5 dark:text-white text-2xl">Thể loại</p>
        </div>
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(category, index) in categories.data" :key="index"
                            ...
                            <td class="px-6 py-4 ">
                                <div class="flex items-center">
                                    <a v-if="queryParam != 'trash'" type="button" @click="handleModal(category)"
                                    ...
                                    <a @click="handleRemove(category.id)" class="cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" data-slot="icon" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round"
