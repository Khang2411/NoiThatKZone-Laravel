...
const handleAction = (action) => {
    console.log(action)
    router.post(route('admin.review.action'), {
        action: action,
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg flex-1">
            ...
            <div class="relative overflow-x-auto">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(review, index) in  reviews.data" :key="index"
                                ...
                                <div
                                    ...
                                    <div class="ps-3">
                                        <div v-if="review.user.role_id !== null">
                                            <span
                                                class="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">Quản
                                                trị viên</span>
                                        </div>
                            ...
                            <td class="px-6 py-4 w-58">
                                {{ review.content }}
                                <div v-for="(reply, index) in review.replies" :key="index" class="p-1">
                                    <span class="ml-4 text-sky-400">{{ reply.content }}</span>
                                </div>
                            </td>

                            <td class="">
                                <span v-if="review.status === 'confirmed'"
                                    class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">
                                    Đã duyệt
                                </span>

                                <span v-else-if="review.status === 'pending'"
                ...
                review.product.name }}</a>
                            ...
                            <td class="px-6 py-4 ">
                                <div class="flex items-center">
                                    <a v-if="queryParam != 'trash'" type="button" @click="handleModal(review)"
                                    ...
                                    <a @click="handleRemove(review.id)" class="cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" data-slot="icon" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="mt-2 mb-2 px-2">
                <Paginate :links="reviews.links"></Paginate>
            </div>
                ...
                <div class="relative w-full max-w-4xl max-h-full">
                    ...
                    <form @submit.prevent="submit" class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-5 dark:text-white">
                            <div class="mb-5">
                                <InputLabel for="content" value="Đánh giá của khách hàng" />
                            ...
                            <div class="mb-5">
                                <InputLabel for="content" value="Trả lời" />
                                <textarea v-model="form.reply"
                                    class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"></textarea>
                                <InputError class="mt-2" :message="form.errors.reply" />
                            </div>

                            <div>
                                <InputLabel for="role" value="Trạng thái" />

                                <div class="flex flex-wrap gap-5">
                                    <div>
                                        <input type="radio" value="pending" v-model="form.status" />
                                        <label class="ml-2 dark:text-[#B6C6C2]">Không duyệt</label>
                                    </div>

                                    <div>
                                        <input type="radio" value="confirmed" v-model="form.status" />
